#!/usr/bin/env python3
"""
Test to capture actual street history element text
"""
...
def test_street_history_capture():
    """Test to capture actual street history element text"""
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for i, element in enumerate(address_elements):
                try:
                    ...
                    if target_address_lower in element_text.lower():
                        ...
                        # Show the raw HTML structure
                        html_content = element.inner_html()
                        print(f"   HTML: {html_content[:500]}...")
                        
                        # Try to extract individual parts
            ...
            if not found_entries:
                ...
                for i, element in enumerate(address_elements[:3]):
                    try:
                        element_text = element.text_content().strip()
                        print(f"\n📄 Element {i+1} sample:")
                        print(f"   Text: {element_text[:300]}...")
                        
                        # Show the raw HTML structure
                        html_content = element.inner_html()
                        print(f"   HTML: {html_content[:300]}...")
                    except:
                        continue
